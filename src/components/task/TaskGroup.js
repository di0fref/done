            ...
            <button onClick={() => setOpen(!open)} className={'mt-6 mb-2 w-full'}>
                <div className={'flex items-center space-x-1 border-b_'}>
                    ...
                    <div className={'flex-grow text-left'}>
                        <div className={'text-neutral-600 dark:text-neutral-300 font-semibold text-sm'}>{capitalize(t(props.title))} <span className={'text-neutral-400 ml-2 font-normal text-sm'}>{props.count}</span></div>
                    </div>
                </div>
            </button>
            <Collapse style={{
